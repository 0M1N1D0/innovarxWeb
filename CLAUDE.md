...
InnovArx marketing/catalog website — Next.js frontend for a software dev agency. No backend today; all data is local to the repo. Content is in Spanish.
...
npm only — no pnpm/yarn lockfiles. No test runner is set up yet (open decision, see tech-stack.md §6).
...
```
src/
  app/                         # routes/layouts only — assembles features, no business logic
  features/
...
**Rendering:** Server Components by default. Add `"use client"` only at the leaf component that actually needs state/effects/browser events/browser APIs — push the boundary as far down the tree as possible, never on a wrapping container.

**Routes ↔ features map 1:1** (architecture.md §6): `/` → `landing` (implemented, also assembles `services-catalog`), `/servicios` → `services-catalog` (pending), `/contacto` → `contact` (pending). No `pricing` feature/route exists — the landing deliberately does not show prices; don't reintroduce prices into `landing` or `services-catalog` without an explicit `pricing` feature.
...
- Fonts are loaded via `next/font/google` in `src/app/layout.tsx` (Space Grotesk, Manrope, IBM Plex Mono), exposed as CSS variables that `tokens.css` references — do not hardcode font-family strings elsewhere.
