...
**Primer `"use client"` del proyecto — `BrushStroke`:** `src/shared/components/BrushStroke.tsx` (spec `ia-docs/specs/001-brush-animated-large/requirements.md`) usa `IntersectionObserver` para animar su "pintado" al entrar en viewport, una API exclusiva del navegador. Es la excepción a la regla anterior, aplicada exactamente como la regla exige: la frontera queda en la hoja — `Hero` y `HomePage`, que lo importan y renderizan, siguen siendo Server Components. No introduce `NextIntlClientProvider` (el componente es decorativo, no consume el catálogo de mensajes). La lógica del observer se extrajo después a `useViewportCycle` (`src/shared/hooks/`), compartida con los Client Components que siguieron.

**Client Components del proyecto (tres a la fecha):** `BrushStroke` (arriba); `HeroVisual` (`src/features/landing/components/HeroVisual.tsx`, spec `ia-docs/specs/002-image-animated-hero/req-002.md`), que orquesta la entrada de laptop y dashboard delante de la brocha; y `HeroEyebrow` (`src/features/landing/components/HeroEyebrow.tsx`, spec `ia-docs/specs/003-phrase-animation/req-003.md`), que anima el eyebrow del Hero con un revoltijo de letras. Los tres son hojas — ninguno sube la frontera a `Hero` ni a `HomePage`. `HeroEyebrow` es el primero que renderiza **texto traducido** en vez de ser puramente decorativo; la mitigación es que el texto le llega por prop, ya resuelto por `Hero` (Server Component) vía `useTranslations` — `HeroEyebrow` nunca importa el catálogo de mensajes, así que `NextIntlClientProvider` sigue sin ser necesario.
